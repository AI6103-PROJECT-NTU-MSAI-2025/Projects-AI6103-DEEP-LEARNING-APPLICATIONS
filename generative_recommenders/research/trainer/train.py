...
from generative_recommenders.research.data.eval import (
    ...
    add_to_summary_writer, # Function to log metrics to TensorBoard
    eval_metrics_v2_from_tensors, # Main function for calculating evaluation metrics
    get_eval_state, # eval_state: all_item_ids: Set[int], candidate_index: CandidateIndex ,top_k_module: TopKModule
)
...
# Importing data and model components specific to recommender systems

from generative_recommenders.research.modeling.sequential.autoregressive_losses import (
    BCELoss, # Binary Cross-Entropy Loss
    InBatchNegativesSampler, # Return the negative IDs and pre-processed embeddings sampled from the current batch's cache.
    LocalNegativesSampler, # Samples negative item IDs and their corresponding embeddings for the given positive IDs.
    LocalTextNegativesSampler, # Return the randomly sampled negative IDs and their L2-normalized combined embeddings (learned + text projection).
)
...
from generative_recommenders.research.modeling.similarity_utils import (
    get_similarity_function, # Function to get the interaction/similarity
)
...
def setup(rank: int, world_size: int, master_port: int) -> None:
    # Function to set up the distributed environment for PyTorch DDP.
...
def cleanup() -> None:
    # Function to clean up the distributed process group.
...
def get_weighted_loss(
    ...
    # Computes the total weighted loss by summing the main loss and auxiliary losses.
    weighted_loss = main_loss
    for key, weight in weights.items():
        cur_weighted_loss = aux_losses[key] * weight
        # Scale auxiliary loss by its weight.
...
def train_fn(
    ...
    # original embedding settings of HSTU
    ...
    # Combines learnable embeddings with selected text embeddings (Blair/open AI).
    ...
    # Get input feature pre-processor
    ...
    # Initialize the main sequential encoder model
